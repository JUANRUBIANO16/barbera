...
@section('content')
<h1 class="mt-4 px-5">Editar Comprobante</h1>
     ...
     <form action="{{ route('comprobantes.update', ['comprobante' => $comprobante]) }}" method="post">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <select name="id_venta" id="id_venta" class="form-control" required>
                    ...
                    @foreach($ventas as $venta)
                        <option value="{{ $venta->id_venta }}" {{ old('id_venta', $comprobante->id_venta) == $venta->id_venta ? 'selected' : '' }}>
            ...
            <div class="col-md-6">
                ...
                <select name="id_tipo_pago" id="id_tipo_pago" class="form-control" required>
                    ...
                    @foreach($tiposPago as $tipoPago)
                        <option value="{{ $tipoPago->id_tipo_pago }}" {{ old('id_tipo_pago', $comprobante->id_tipo_pago) == $tipoPago->id_tipo_pago ? 'selected' : '' }}>
                            {{ $tipoPago->metodo }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6">
                ...
                <input type="date" name="fecha" id="fecha" class="form-control" value="{{ old('fecha', $comprobante->fecha) }}" required>
                @error('fecha')
                    <small class="text-danger">{{'*'.$message}}</small>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="time" name="hora" id="hora" class="form-control" value="{{ old('hora', $comprobante->hora) }}" required>
                @error('hora')
                    <small class="text-danger">{{'*'.$message}}</small>
                @enderror
            </div>
